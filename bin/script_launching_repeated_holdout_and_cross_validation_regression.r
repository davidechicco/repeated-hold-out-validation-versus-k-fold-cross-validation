...
for dataset in "${datasets[@]}"; do
  ...
  for global_iteration in $number_of_global_interations; do
    ...
    outputFile=$outputFolder"/output_"$dataset"_iterations"$global_iteration"_"$current_date_and_time".txt"
    Rscript repeated_holdout_and_cross_validation_regression.r $global_iteration $dataset >> $outputFile 2>> $outputFile
    tail $outputFile
  done
done
